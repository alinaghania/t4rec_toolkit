...
class DataValidator:
    ...
    def validate_input_data(self, 
                          ...
                          target_column: Optional[str] = None) -> ValidationResult:
        """
    ...
    def validate_sequence_data(self, 
                             data: pd.DataFrame, 
                             sequence_columns: List[str],
                             expected_length: Optional[int] = None) -> ValidationResult:
        """
        ...
        for col in sequence_columns:
            ...
            col_stats = self._validate_sequence_column(data[col], col, result, expected_length)
            sequence_stats[col] = col_stats
        
        result.statistics['sequence_stats'] = sequence_stats
        return result
    
    def validate_categorical_data(self, 
                                data: pd.DataFrame, 
                                categorical_columns: List[str],
                                max_categories: Optional[int] = None) -> ValidationResult:
        """
    ...
    def _validate_basic_structure(self, data: pd.DataFrame, result: ValidationResult):
        ...
        if duplicate_cols:
            result.add_error(
                'duplicate_columns',
                f"Colonnes dupliquées: {duplicate_cols}"
            )
    ...
    def _validate_sequence_column(self, 
                                series: pd.Series, 
                                col_name: str, 
                                result: ValidationResult,
    ...
    def _validate_categorical_column(self, 
                                   series: pd.Series, 
                                   col_name: str, 
                                   result: ValidationResult,
                                   max_categories: Optional[int] = None) -> Dict[str, Any]:
    ...
    def validate_sequence_consistency(self, 
                                    sequences: Dict[str, np.ndarray]) -> ValidationResult:
        """
    ...
    def validate_sequence_values(self, 
                                sequence: np.ndarray, 
                                sequence_name: str,
    ...
    def validate_schema(self, schema: Dict[str, Any]) -> ValidationResult:
        ...
        if 'feature_specs' in schema:
            ...
            result.statistics.update({
                ...
                'sequence_features': sum(1 for spec in specs if spec.get('is_sequence', False)),
                'categorical_features': sum(1 for spec in specs if not spec.get('is_sequence', False) and spec.get('dtype') == 'int32'),
                'numerical_features': sum(1 for spec in specs if spec.get('dtype') == 'float32')
            })
        
        return result
    
    def _validate_feature_specs(self, 
                                feature_specs: List[Dict[str, Any]], 
                                result: ValidationResult):
        """Valide les spécifications des features."""
        if not isinstance(feature_specs, list):
            result.add_error(
                'invalid_feature_specs_type',
                "feature_specs doit être une liste"
            )
            return
        ...
        if len(feature_specs) == 0:
            result.add_error(
                'empty_feature_specs',
                "Aucune spécification de feature fournie"
            )
            return
        ...
        for i, spec in enumerate(feature_specs):
            if not isinstance(spec, dict):
                result.add_error(
                    'invalid_spec_type',
                    f"Spécification {i} doit être un dictionnaire"
                )
                continue
            ...
            if 'name' in spec:
                ...
                if name in feature_names:
                    result.add_error(
                        ...
                        f"Nom de feature dupliqué: {name}"
                    )
